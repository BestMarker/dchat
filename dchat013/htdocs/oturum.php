...
if($query_check->num_rows >= 1 and isset($_SESSION["uid"])) {
    header("Location: ..");
    die();
} elseif ($query_check->num_rows >= 1 & !isset($_SESSION['uid'])){
...
<div class="budiv">
	<form method="post" action="oturum.php" class="form-horizontal">
		...
		<button type="submit" name="submit" class="btn btn-primary">Hesap Oluştur</button>

	</form>
	<p class="uyariyazisi">Hesabı oluşturduğunuzda ip ile hesap bağlanacak!</p>
</div>
...
if (isset($_POST['username']) and isset($_POST['pass'])) {
	...
	$query_1 = "INSERT INTO kullaniciip (ip, kullaniciadi , kullanicisifre) VALUES ('$uip', '$ism', '$pass')";
    $query_run = mysqli_query($con, $query_1);
	
	if ($query_run) { 
		$hesapolustumu = "SELECT * FROM `kullaniciip` WHERE `kullaniciadi` = '$ism'";
		$hesapolustumucheck = mysqli_query($con, $query_perm);
		$hesapolustumusonuc = mysqli_fetch_assoc($hesapolustumucheck);

		$_SESSION['username'] = $hesapolustumusonuc['kullaniciadi'];
		$_SESSION["uid"] = $hesapolustumusonuc['id'];
    	header("Location: ..");
    	die();
	}
}
